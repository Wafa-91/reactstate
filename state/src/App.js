
import './App.css';

import React, { Component } from 'react'

export class App extends Component {

  // state exmple1
  // state={num:0}
  // incr=()=>{this.setState({num:this.state.num+1})}
  // decr=()=>{this.setState({num:this.state.num-1})}
  // state exmple2chekpont
  // state={fullName:"AHMED",
  // imgSrc:"https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
  //  profession:"DEVELLOPR",
  // show:true}
  // statshow=()=>this.setState({show:!this.state.show})

  // state exmple3
// state={tabimag:["https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500",
//                 "data:image/jpeg;base64,",
//               "https://media.gettyimages.com/photos/portrait-of-a-cheerful-young-man-picture-id640021202?s=612x612"],
//             index:0}
    
//      nextimag=()=>{this.setState({index:this.state.index+1})} 
//      sameimg=()=>{this.setState({index:this.state.index})}  

// state exmple4
// state={tab:[{name:"ahmed",srcimg:"https://media.gettyimages.com/photos/portrait-of-a-cheerful-young-man-picture-id640021202?s=612x612"},{name:"ali",srcimg:"https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500"}],index:0}
  render() {
    return (
      <div className="App">

        {/* exemple1 */}
        {/* <h1>{this.state.num}</h1>
         <button onClick={this.incr}>+</button> 
        <button onClick={()=>{this.setState({num:(this.state.num<10)? this.state.num+1:this.state.num+10})}}>+</button>
        <button onClick={()=>this.setState({num:this.state.num-1})}>-</button> */}

{/* exemple2 */}
{/* <button onClick={this.statshow} style={{marginTop:"50px"}}>show</button>
{this.state.show ?<div>
<img src={this.state.imgSrc} style={{width:"200px",marginTop:"10px"}}></img>
<h2>MY NAME IS {this.state.fullName}</h2>
<h2>IM A {this.state.profession}</h2> 
</div>:<h1>click button to show  person's profile</h1> } */}

{/* exemple3 */}
    {/* <img style={{with:"100px",height:"100px"}} src={this .state.tabimag[this.state.index]}></img>
    <br/>
    <button onClick={this.state.index <2 ?this.nextimag :this.sameimg}>next image</button> */}

    {/* exmple4 */}


{/* {this.state.tab.map((elm,curntindex)=><button onClick={()=>this.setState({index:curntindex})}>{elm.name}</button>)} 

<h1> {this.state.tab[this.state.index].name}</h1>
<img src={this.state.tab[this.state.index].srcimg}></img> */}

      </div>
    )
  }
}

export default App

